...
async function generateScriptWithGemini({ prompt, brandName, brandDesc, tone, durationSeconds, rrss, creativity, geminiApiKey, brandFull = null }) {
  ...
  for (const m of GEMINI_MODELS) {
    try {
      model = genAI.getGenerativeModel({ model: m, generationConfig: { temperature, maxOutputTokens: 2048, responseMimeType: 'application/json' } });
...
// ─── Script Generation Engine v3 — PAS Cinematic Copywriter ─────────────────

// ── Industry detection ──────────────────────────────────────────────────────
function detectCategory(text) {
  const t = (text || '').toLowerCase();
  if (/agua|filtro|purif|jarra|bidón|dispensad|purificad/i.test(t)) return 'water';
  if (/restaurante|menú|comida|gastro|delivery|food|mesa|reserva/i.test(t)) return 'food';
  if (/clínica|médico|salud|bienestar|nutrici|fitness|gym/i.test(t)) return 'health';
  if (/inmobili|propiedad|arriend|casa|depart|real.?estate/i.test(t)) return 'realestate';
  if (/tienda|ecommerce|producto.*online|shop|compra/i.test(t)) return 'ecommerce';
  if (/curso|aprender|educaci|formaci|academia/i.test(t)) return 'education';
  if (/crm|lead|pipeline|sales.?soft/i.test(t)) return 'crm';
  if (/app|saas|software|plataforma|automatiz/i.test(t)) return 'saas';
  return 'general';
}

// ── Cinematic visual prompts per category + scene type ──────────────────────
const VISUAL_LIBRARY = {
  water: {
    hook:    () => `Ultra slow-motion macro: single water droplet exploding on crystal surface, backlit with cyan-blue light rays, suspended droplets in mid-air, 4K cinematic, photorealistic, dramatic lighting, no text`,
    brand:   (b) => `Premium water purifier jug hero shot on white marble counter, soft studio lighting, clean minimalist background, ${b} aesthetic, reflections, product photography masterclass`,
    feature: () => `Scientific cross-section 3D render: 5-layer water filtration system, particles being captured, clean blue aesthetic, technical precision, no text overlays`,
    cta:     () => `Happy family drinking fresh water at breakfast table, warm golden morning light streaming through window, lifestyle photography, aspirational, warm tones`,
  },
  food: {
    hook:    () => `Extreme close-up: gourmet dish being plated by chef hands, steam rising dramatically, warm golden bokeh background, food photography, cinematic`,
    brand:   (b) => `Elegant restaurant full of happy diners, warm amber candlelight, premium atmosphere, cinematic wide shot, aspirational`,
    feature: () => `Chef using modern tablet POS in professional kitchen, motion blur background, technology meets gastronomy`,
    cta:     () => `Couple smiling over romantic dinner, glasses clinking, beautiful meal, golden hour soft light, lifestyle`,
  },
  health: {
    hook:    () => `Person looking in mirror with transformation expression, dramatic side lighting, emotional close-up, before-after feeling`,
    brand:   (b) => `Clean modern wellness center, natural light, plants, professional staff smiling, trust and care, ${b} branding aesthetic`,
    feature: () => `Doctor using digital health tablet, patient interaction, cinematic blue tones, trust and professionalism`,
    cta:     () => `Person celebrating fitness achievement outdoors, sunrise backdrop, arms raised, energy and vitality, inspirational`,
  },
  general: {
    hook:    () => `Dramatic reveal: problem being solved at climax moment, strong contrast cinematic lighting, tension and release, 4K`,
    brand:   (b) => `Clean product/service hero reveal, professional studio lighting, premium modern aesthetic, cinematic composition`,
    feature: () => `Professional using cutting-edge technology in action, dynamic composition, modern workflow, motion blur`,
    cta:     () => `Successful entrepreneur celebrating results, warm aspirational photography, confidence and achievement, lifestyle`,
  },
};

function buildVisualPrompt(sceneType, brandName, category, tone) {
  const lib = VISUAL_LIBRARY[category] || VISUAL_LIBRARY.general;
  const fn  = lib[sceneType] || lib.hook;
  const toneAdd = {
    Urgente:     'urgent dramatic lighting, high contrast shadows, tension',
    Inspirador:  'warm golden hour tones, uplifting ethereal mood',
    Profesional: 'clean crisp corporate, minimal, precision',
    Divertido:   'vibrant saturated colors, playful kinetic energy',
    Dramático:   'moody cinematic, deep blue shadows, noir atmosphere',
  }[tone] || 'cinematic quality';
  return `${fn(brandName)}, ${toneAdd}, no text overlays, no watermarks, 16:9 cinematic`;
}

function buildSfxInstruction(sceneType) {
  return {
    hook:    'Deep cinematic bass impact at 0s, tension whoosh 0.5s — grab attention immediately',
    brand:   'Rising synth swell, bright premium logo-reveal chime tone',
    feature: 'Subtle clean UI tick/click, light tech notification whoosh',
    cta:     'High-energy impact boom + bright sparkle, logo sting ending — maximum impact',
  }[sceneType] || 'Ambient cinematic sound, matching scene energy level';
}

// ── PAS Narrative Copy per Industry + Tone ──────────────────────────────────
const PAS = {
  water: {
    P: {
      Urgente:     [(b,p) => `¿Sabes exactamente qué hay en el agua que bebe tu familia ahora mismo? Cloro. Metales pesados. Bacterias. Invisible. Peligroso.`,
                   (b,p) => `El agua del grifo "limpia" contiene hasta 47 contaminantes detectados por la OMS. Hoy. En tu casa.`],
      Inspirador:  [(b,p) => `Imagina abrir la llave y sentir confianza absoluta. Que cada sorbo sea puro. Sin dudas. Sin sabor a química. Solo agua real.`,
                   (b,p) => `El agua es vida. Pero no toda el agua es igual. Tu familia merece lo mejor: pura, cristalina, certificada.`],
      Profesional: [(b,p) => `Los estándares de calidad del agua potable se incumplen en el 68% de los hogares. Datos de la OMS, 2024.`],
      Dramático:   [(b,p) => `Cada día, sin saberlo, tu familia bebe lo que miles de tuberías viejas dejan pasar. El problema no se ve. El daño sí.`],
      Divertido:   [(b,p) => `Spoiler: el agua que llega a tu grifo hizo un viaje de dudosa higiene antes de llegar a ti. No tan apetecible, ¿verdad?`],
    },
    A: {
      default:     [(b) => `El cloro irrita la piel y el estómago. Los metales pesados se acumulan. Los filtros baratos no eliminan lo que importa. Y tu familia bebe eso cada día.`],
    },
    S: {
      Urgente:     [(b) => `${b} lo resuelve hoy. Filtración en 5 etapas certificada por la OMS. El 99.9% de contaminantes, eliminados. Permanentemente.`,
                   (b) => `${b}: tecnología de purificación de última generación, para tu hogar, en diseño premium que cabe en cualquier cocina.`],
      Inspirador:  [(b) => `${b} nació para cambiar eso. Agua purificada en 5 etapas certificadas, sin plásticos de un solo uso. Pura. Real. Tuya.`,
                   (b) => `Con ${b}, cada gota que bebe tu familia pasó por 5 barreras de purificación. El resultado se ve, se siente y se saborea.`],
      default:     [(b) => `${b}: purificación certificada, diseño premium, instalación en 2 minutos. Sin técnicos. Sin complicaciones. Solo agua pura.`],
    },
    F: { default: [(f,b) => `${b} incluye ${f.toLowerCase()}: porque calidad y comodidad no deberían ser una elección. Las tienes ambas.`,
                  (f,b) => `${f} en ${b}: probado, certificado y listo para usar desde el primer día.`] },
    CTA: {
      Urgente:     [(b) => `Entra a ${b.toLowerCase().replace(/\s+/g,'')}.com ahora. 30 días sin riesgo. Tu familia no puede esperar más.`],
      Inspirador:  [(b) => `Prueba ${b} 30 días. Si no sientes la diferencia, te devolvemos cada centavo. Pero la sentirás. Garantizado.`],
      default:     [(b) => `Visita ${b.toLowerCase().replace(/\s+/g,'')}.com. Envío gratis hoy. 30 días de prueba sin riesgo. Agua pura, ahora mismo.`],
    },
  },
  food: {
    P: { default: [(b,p) => `Mesas llenas. Comanda tras comanda. El equipo desbordado. Y los clientes... esperando. Esto no puede seguir así.`] },
    A: { default: [(b) => `Errores en pedidos. Tiempo perdido en papel. El 40% de los restaurantes fracasan por mala gestión, no por la comida.`] },
    S: { default: [(b) => `${b} digitaliza tu restaurante en un día. Comandas, caja, delivery e inventario, todo desde un tablet.`] },
    F: { default: [(f,b) => `${f} en ${b}: menos errores, más mesas y clientes que vuelven. Así de simple.`] },
    CTA: { default: [(b) => `Prueba ${b} gratis 14 días. Sin contratos. Sin complicaciones. Tu restaurante, transformado.`] },
  },
  general: {
    P: {
      Urgente:     [(b,p) => `¿Cuánto dinero está perdiendo tu negocio cada mes por ${p}? La respuesta duele. Pero tiene solución.`,
                   (b,p) => `Tu competencia ya lo resolvió. Mientras usas el mismo sistema de siempre, ellos crecen. Hora de cambiar eso.`],
      Inspirador:  [(b,p) => `Hay negocios que crecen sin esfuerzo y negocios que pelean contra la corriente. La diferencia está en las herramientas.`,
                   (b,p) => `¿Y si ${p} dejara de ser un problema para siempre? Con ${b}, ese futuro ya existe. Hoy.`],
      Profesional: [(b,p) => `El 78% de las empresas que no digitalizan sus procesos pierden competitividad en menos de 2 años.`],
      Divertido:   [(b,p) => `Seamos honestos: ${p} es terrible. Como terrible-terrible. Pero buenas noticias: ${b} existe.`],
      Dramático:   [(b,p) => `En el mercado de hoy, ${p} no es un inconveniente. Es una sentencia. ${b} es el indulto.`],
    },
    A: {
      default:     [(b) => `Cada día sin la herramienta correcta es un día de ingresos perdidos, tiempo robado y oportunidades que se van a la competencia.`],
      Urgente:     [(b) => `Mientras esperas el momento perfecto, tus competidores ya usan la tecnología que tú todavía no tienes.`],
    },
    S: {
      Urgente:     [(b) => `${b} lo cambia hoy. Configuración en minutos. Resultados desde el primer uso. Sin curva de aprendizaje. Sin excusas.`],
      Inspirador:  [(b) => `${b} nació para quitarte ese peso. Automatiza lo repetitivo, potencia lo que importa y te devuelve tiempo para crecer.`],
      Profesional: [(b) => `${b} es la plataforma que simplifica, conecta y optimiza cada proceso. Implementación guiada. ROI medible.`],
      Divertido:   [(b) => `${b} es básicamente un empleado que nunca descansa, nunca se queja y siempre tiene todo actualizado. Sin el drama.`],
      Dramático:   [(b) => `${b} es la herramienta que las empresas líderes del mañana ya usan hoy. ¿Tu negocio estará entre ellas?`],
      default:     [(b) => `${b} resuelve el problema desde la raíz: automatización inteligente, integración total, resultados desde el día uno.`],
    },
    F: { default: [(f,b) => `${f} en ${b}: precisión, velocidad y control que antes solo tenían las grandes empresas.`,
                  (f,b) => `Con ${f}, ${b} te da el poder de decidir basado en datos reales, no en intuición.`] },
    CTA: {
      Urgente:     [(b) => `Entra a ${b.toLowerCase().replace(/\s+/g,'')}.com ahora. 14 días gratis. Sin tarjeta. El momento es HOY.`],
      Inspirador:  [(b) => `Tu próximo gran salto empieza hoy. Únete a ${b} gratis y descubre lo que tu negocio puede lograr sin límites.`],
      default:     [(b) => `Empieza gratis en ${b.toLowerCase().replace(/\s+/g,'')}.com. Sin compromiso. Con soporte desde el primer día.`],
    },
  },
};

function pasCopy(section, tone, category, brandName, problem, feature) {
  const cat = PAS[category] || PAS.general;
  const sec = cat[section] || PAS.general[section] || {};
  const variants = sec[tone] || sec.default || PAS.general[section]?.[tone] || PAS.general[section]?.default || [(x) => x];
  const hash = strHash((brandName||'') + (tone||'') + (problem||'') + (feature||''));
  const fn = variants[Math.abs(hash) % variants.length];
  if (section === 'F') return fn(feature || 'la funcionalidad', brandName);
  if (section === 'CTA') return fn(brandName);
  return fn(brandName, problem);
}

// ── PAS Scene Builder ───────────────────────────────────────────────────────
function buildScenesPAS({ brandName, problem, features, benefit, category, tone, sceneCount, rrss, hash, boldness }) {
  const scenes = [];
  let cumTime = 0;
  const sceneLen = sceneCount > 0 ? Math.floor(21 / sceneCount) : 5;

  // 1. HOOK — Problem (P)
  const hookVoice = pasCopy('P', tone, category, brandName, problem, null);
  scenes.push({
    type: 'hook',
    title: tone === 'Urgente' ? '¡Atención!' : tone === 'Dramático' ? 'La realidad.' : '¿Te pasa esto?',
    subtitle: `Hook visual para ${rrss}`,
    badge: `📍 ${rrss}`,
    painPoints: buildPainPoints(problem, tone, category),
    voiceover: hookVoice,
    visual_prompt: buildVisualPrompt('hook', brandName, category, tone),
    sfx_instruction: buildSfxInstruction('hook'),
    timestamp: cumTime,
  });
  cumTime += sceneLen;

  // 2. AGITATION (only if ≥4 scenes)
  if (sceneCount >= 4) {
    const agitVoice = pasCopy('A', tone, category, brandName, problem, null);
    scenes.push({
      type: 'brand',
      title: `${brandName} | La solución`,
      subtitle: benefit,
      badge: tone === 'Profesional' ? '🏆 Certificado' : tone === 'Urgente' ? '⚡ Disponible ahora' : '✨ La diferencia',
      items: buildBrandStats(problem + ' ' + benefit + ' ' + category, benefit),
      voiceover: pasCopy('S', tone, category, brandName, problem, null),
      visual_prompt: buildVisualPrompt('brand', brandName, category, tone),
      sfx_instruction: buildSfxInstruction('brand'),
      timestamp: cumTime,
    });
    cumTime += sceneLen;
  }
  ...
  // 3. FEATURES
  const featCount = Math.max(0, sceneCount - (sceneCount >= 4 ? 2 : 1) - 1);
  for (let i = 0; i < Math.min(featCount, features.length); i++) {
    const feat = features[i];
    scenes.push({
      type: 'feature',
      title: feat,
      subtitle: `${brandName} en acción`,
      badge: ['⚡ Tecnología', '📊 Resultados', '🚀 Rendimiento', '🔒 Confianza'][i % 4],
      items: [['Sin configuración', 'Resultado inmediato', 'Incluido gratis'], ['Fácil de usar', '100% automatizado', 'Soporte 24/7'], ['Plug & play', 'Sin código', 'Escala contigo']][i % 3],
      voiceover: pasCopy('F', tone, category, brandName, problem, feat),
      visual_prompt: buildVisualPrompt('feature', brandName, category, tone),
      sfx_instruction: buildSfxInstruction('feature'),
      timestamp: cumTime,
    });
    cumTime += sceneLen;
  }

  // 4. CTA — Solution payoff
  scenes.push({
    type: 'cta',
    title: tone === 'Urgente' ? '¡Empieza AHORA!' : tone === 'Dramático' ? 'Es tu momento.' : 'Empieza | Hoy',
    subtitle: benefit,
    badge: '🎯 Comenzar Gratis',
    items: [`${brandName.toLowerCase().replace(/\s+/g,'')}.com`, '✓ Sin tarjeta de crédito'],
    voiceover: pasCopy('CTA', tone, category, brandName, problem, null),
    visual_prompt: buildVisualPrompt('cta', brandName, category, tone),
    sfx_instruction: buildSfxInstruction('cta'),
    timestamp: cumTime,
  });
...
// ── Pain points — industry-aware ──────────────────────────────────────────────
function buildPainPoints(problem, tone, category) {
  const emojiSets = {
    Urgente: ['🚨','💸','😤'], Inspirador: ['🌙','😮‍💨','🕐'],
    Profesional: ['📉','⏱','🔧'], Divertido: ['😵','🤦','😩'], Dramático: ['💀','🔥','⚠️'],
  };
  const emojis = emojiSets[tone] || emojiSets.Inspirador;

  const categoryPains = {
    water:     ['Agua con cloro y mal sabor', 'Contaminantes invisibles', 'Plástico de un solo uso'],
    food:      ['Comandas en papel', 'Errores en pedidos', 'Mesas sin gestión'],
    health:    ['Sin seguimiento personalizado', 'Resultados lentos', 'Motivación que baja'],
    general:   [problem.substring(0, 28), 'Oportunidades perdidas', 'Tiempo sin optimizar'],
  };
  const pains = (categoryPains[category] || categoryPains.general);
  return emojis.map((emoji, i) => ({ emoji, text: pains[i] || problem.substring(0, 28) }));
}

// ─── Old TONE_HOOKS (kept for backward compat) ──────────────────────────────
const TONE_HOOKS = {
  Urgente: [
    (b, p) => `¿Cuánto dinero está perdiendo tu negocio por ${p}? La respuesta te va a doler. ${b} lo cambia hoy.`,
    (b, p) => `Tu competencia ya resolvió ${p}. Tú, ¿todavía esperas? ${b} es la razón por la que ellos crecen y tú no.`,
    (b, p) => `Alerta: el 68% de los negocios con ${p} no sobreviven 2 años. ${b} existe para que tú seas la excepción.`,
    (b, p) => `Cada hora sin resolver ${p} es facturación tirada a la basura. ${b} para eso. Ahora.`,
  ],
  Inspirador: [
    (b, p) => `¿Y si ${p} dejara de ser un problema para siempre? Con ${b}, ese futuro ya es posible. Hoy.`,
    (b, p) => `Los mejores negocios no aceptan límites. Superaron ${p} y siguieron creciendo. Con ${b}, tú también puedes.`,
    (b, p) => `Imagina llegar al lunes sin preocuparte por ${p}. ${b} hace exactamente eso por miles de emprendedores.`,
    (b, p) => `Hay dos tipos de negocios: los que luchan con ${p}, y los que usan ${b}. ¿Cuál quieres ser?`,
  ],
  Profesional: [
    (b, p) => `Las empresas que optimizaron ${p} aumentaron su rentabilidad en un 40%. ${b} es cómo lo hicieron.`,
    (b, p) => `${p} no es un problema de esfuerzo. Es un problema de herramientas. ${b} es la herramienta correcta.`,
    (b, p) => `Datos, eficiencia, resultados. ${b} fue diseñado para negocios que no toleran ${p}.`,
    (b, p) => `Los mejores equipos no trabajan más duro. Trabajan más inteligente. ${b} elimina ${p} con precisión.`,
  ],
  Divertido: [
    (b, p) => `Seamos honestos: ${p} es horrible. Como horrible-horrible. Pero buenas noticias: ${b} existe.`,
    (b, p) => `Spoiler: ${p} no tiene por qué ser tu realidad. ${b} llegó para arruinar ese chiste.`,
    (b, p) => `¿Sabes qué tiene en común tu negocio con el 99% del planeta? Odian ${p}. ${b} odia eso también.`,
    (b, p) => `${p}? Clásico. Pero ya en 2025, con ${b} al lado, eso ya no debería pasarte.`,
  ],
  Dramático: [
    (b, p) => `En el mercado de hoy, ${p} no es un inconveniente. Es una sentencia. ${b} es el indulto.`,
    (b, p) => `Hay negocios que cerraron por no resolver ${p} a tiempo. Los que sobrevivieron tenían ${b}.`,
    (b, p) => `${p} no avisa. Destruye en silencio. ${b} lo detecta, lo ataca y lo elimina. Sin piedad.`,
    (b, p) => `Este es el momento. O resuelves ${p} con ${b}, o el mercado lo resuelve por ti.`,
  ],
};

const TONE_BRAND_LINES = {
  Urgente: [
    (b, ben) => `${b} no es una opción más. Es la única opción que realmente funciona. ${ben}. Sin excusas.`,
    (b, ben) => `Miles de negocios eligieron ${b} y triplicaron sus resultados. ${ben}. El tuyo puede ser el siguiente.`,
    (b, ben) => `${b}: la plataforma que hace en minutos lo que antes te tomaba días. ${ben}. Ya.`,
  ],
  Inspirador: [
    (b, ben) => `${b} nació para redefinir lo que es posible en tu industria. ${ben}. Sin límites.`,
    (b, ben) => `Con ${b}, cada decisión tiene datos. Cada proceso tiene ritmo. ${ben}. Así se crece.`,
    (b, ben) => `${b} es más que una herramienta. Es el socio que tu negocio siempre necesitó. ${ben}.`,
  ],
  Profesional: [
    (b, ben) => `${b} integra todo lo que tu equipo necesita en una sola plataforma. ${ben}. Medible desde el día uno.`,
    (b, ben) => `Precisión, automatización, resultados. ${b} fue construido para negocios serios. ${ben}.`,
    (b, ben) => `${b} no promete. Entrega. ${ben}. Con métricas reales, en tiempo real.`,
  ],
  Divertido: [
    (b, ben) => `${b} lo resolvió todo. Literal. ${ben}. Sí, así de fácil. No, no es trampa.`,
    (b, ben) => `¿Lo mejor de ${b}? Que funciona aunque no seas experto en nada. ${ben}. Bienvenido al futuro.`,
    (b, ben) => `${b}: porque la vida es demasiado corta para no tener ${ben}. Ya dije lo que dije.`,
  ],
  Dramático: [
    (b, ben) => `${b}. Una plataforma. Un propósito. ${ben}. La diferencia entre existir y dominar.`,
    (b, ben) => `En el nuevo orden del mercado, ${b} es poder. ${ben}. Los que lo tienen, ganan.`,
    (b, ben) => `${b} no vino a mejorar las cosas. Vino a cambiarlas para siempre. ${ben}. Bienvenido.`,
  ],
};

const TONE_FEATURE_LINES = {
  Urgente: [
    (f, b) => `${f}. Con ${b}, está listo en minutos. Tu competencia aún lo configura a mano.`,
    (f, b) => `¿${f}? ${b} lo hace automático. Tú solo cobras.`,
    (f, b) => `${f} en ${b}: porque perder tiempo es perder dinero. Punto.`,
  ],
  Inspirador: [
    (f, b) => `${f} con ${b} se convierte en tu mejor vendedor. Sin descanso. Sin quejarse.`,
    (f, b) => `${b} te da ${f} para que tú te enfoques en lo que importa: crecer.`,
    (f, b) => `Con ${f}, ${b} convierte datos en decisiones y decisiones en crecimiento.`,
  ],
  Profesional: [
    (f, b) => `${f}: implementado en ${b} con estándares enterprise. Escalable desde el día uno.`,
    (f, b) => `${b} ofrece ${f} con trazabilidad completa y reportes en tiempo real.`,
    (f, b) => `${f} en ${b}: precisión técnica al servicio de tu operación.`,
  ],
  Divertido: [
    (f, b) => `¿Lo mejor de ${b}? ${f}. Por fin alguien lo hizo bien.`,
    (f, b) => `${f} en ${b}. Sencillo. Rápido. Sin tutoriales de 3 horas. Te lo juro.`,
    (f, b) => `${b} tiene ${f} y es tan fácil que hasta tu jefe puede usarlo. Guiño.`,
  ],
  Dramático: [
    (f, b) => `${f}: la herramienta que solo ${b} pone en tus manos. El resto, copiará después.`,
    (f, b) => `Con ${f}, ${b} te da el poder que antes solo tenían las grandes corporaciones.`,
    (f, b) => `${b} convirtió ${f} en arma competitiva. Ahora es tuya.`,
  ],
};

const TONE_CTA_LINES = {
  Urgente: [
    (b) => `El momento es AHORA. Entra en ${b} gratis. Sin tarjeta. Sin burocracia. Solo resultados.`,
    (b) => `Cada segundo que esperas es ventaja que le das a tu competencia. ${b} gratis. Entra ya.`,
    (b) => `¡${b} te da los primeros 30 días gratis! Después decides. Pero empieza hoy.`,
  ],
  Inspirador: [
    (b) => `Tu próximo gran salto empieza con un clic. Únete a ${b} gratis y descubre lo que es posible.`,
    (b) => `Miles ya transformaron su negocio con ${b}. Tu historia de éxito empieza hoy. Gratis.`,
    (b) => `No esperes el momento perfecto. Crea el momento perfecto. ${b} te da las herramientas. Tú pones la visión.`,
  ],
  Profesional: [
    (b) => `Agenda tu demo gratuita con ${b} hoy. Sin compromiso. Con resultados medibles desde el inicio.`,
    (b) => `${b} ofrece 14 días gratis con acceso completo. Mídelo, compruébalo, decide. Sin riesgo.`,
    (b) => `Comienza tu prueba gratuita en ${b}. Tu equipo lo agradecerá. Tu CFO también.`,
  ],
  Divertido: [
    (b) => `Dale una oportunidad a ${b}. Es gratis, es fácil y spoiler: lo vas a amar.`,
    (b) => `Último capítulo: entras a ${b} gratis, usas 5 minutos y nunca más vuelves al caos. Fin.`,
    (b) => `Serio, ¿qué tienes que perder? ${b} es gratis para empezar. Y cambia todo. Prometido.`,
  ],
  Dramático: [
    (b) => `El futuro de tu negocio se decide hoy. ${b} gratis. Sin segunda oportunidad para el mercado.`,
    (b) => `No hay marcha atrás en el mercado que viene. ${b} o quedarte atrás. La elección es tuya. Hoy.`,
    (b) => `${b} ya transformó miles de negocios. El tuyo es el siguiente. Si decides actuar ahora.`,
  ],
};

// ── Seeded random picker (deterministic per prompt so script is stable) ─────────
...
function extractProblem(prompt) {
  const p = prompt;
  const patterns = [
    /(?:sin|falta\s+de|problema\s+con|dificultad\s+para|pérdida\s+de|gestión\s+de)\s+([^.?!,\n]{5,55})/i,
    /(?:no\s+puedes?|no\s+logras?|no\s+tienen?|no\s+consigues?)\s+([^.?!,\n]{5,55})/i,
    /(?:cansado|harto|frustrado|agotado)\s+(?:de\s+)?([^.?!,\n]{5,55})/i,
    /(?:mejorar|optimizar|resolver|solucionar|automatizar)\s+([^.?!,\n]{5,55})/i,
    /(?:para)\s+([^.?!,\n]{5,55})/i,
  ];
  for (const pat of patterns) {
    ...
    if (m) {
      ...
      found = found.replace(/\s+(y|o|de|que|con|para|en)$/i, '').trim();
      return found.substring(0, 52);
    }
  }
  // Fall back: take the most descriptive noun phrase from the prompt
  const words = p.trim().split(/\s+/);
  return words.slice(0, 7).join(' ') || 'los procesos que frenan tu negocio';
}

function extractBenefit(prompt) {
  const p = prompt;
  const patterns = [
    /(?:para\s+que|para\s+poder)\s+([^.?!,\n]{5,65})/i,
    /(?:permite|ayuda\s+a|te\s+permite|logra|consigue)\s+([^.?!,\n]{5,65})/i,
    /(?:mejora|aumenta|optimiza|reduce|automatiza|incrementa)\s+([^.?!,\n]{5,65})/i,
    /(?:resultados?|beneficio|ventaja)\s+[^.?!,\n]*?[:es]\s+([^.?!,\n]{5,65})/i,
  ];
  for (const pat of patterns) {
    const m = p.match(pat);
    if (m) return m[1].trim().replace(/\s+(y|o|de|que)$/i, '').trim().substring(0, 60);
  }
  // Extract product category/purpose
  if (/agua|filtro|purifica/i.test(p)) return 'agua pura para toda tu familia';
  if (/delivery|envío|pedido/i.test(p)) return 'pedidos en tiempo récord';
  if (/ventas?|crm|cliente/i.test(p)) return 'más ventas y clientes satisfechos';
  if (/analí|dato|report/i.test(p)) return 'decisiones basadas en datos reales';
  if (/chat|comunicación|mensaj/i.test(p)) return 'comunicación fluida con tus clientes';
  return 'resultados reales desde el primer día';
}

function extractFeatures(prompt) {
  const p = prompt.toLowerCase();
  const features = [];
  if (/chat|mensaj|comunicaci|atención/i.test(p)) features.push('Chat en tiempo real');
  if (/auto|automáti|robot|bot/i.test(p)) features.push('Automatización inteligente');
  if (/analí|report|dato|estadístic|métric/i.test(p)) features.push('Analytics en tiempo real');
  if (/pago|cobro|factura|billing|compra/i.test(p)) features.push('Pagos integrados');
  if (/cliente|crm|contact|lead/i.test(p)) features.push('CRM completo');
  if (/pedido|orden|delivery|envío|logística/i.test(p)) features.push('Gestión de pedidos');
  if (/ia|inteligencia|ai|gpt|smart/i.test(p)) features.push('IA integrada');
  if (/app|móvil|mobile|celular/i.test(p)) features.push('App móvil nativa');
  if (/integra|conect|api|sinc/i.test(p)) features.push('Integración total');
  if (/equipo|team|colabor|usuario/i.test(p)) features.push('Colaboración en equipo');
  if (/filtro|purifica|calidad|agua/i.test(p)) features.push('Filtración certificada');
  if (/salud|bienestar|nutrici/i.test(p)) features.push('Certificado de calidad');
  if (/restaurante|menú|reserva|mesa/i.test(p)) features.push('Gestión de mesas y reservas');
  if (/inventario|stock|almacén/i.test(p)) features.push('Control de inventario');
  if (features.length === 0) {
    features.push('Panel de control intuitivo', 'Automatización total', 'Soporte 24/7');
  }
  return features.slice(0, 4);
}

// (buildPainPoints v3 defined above in PAS engine section)

// Build brand stats that match the product
function buildBrandStats(prompt, benefit) {
  const p = prompt.toLowerCase();
  if (/agua|filtro|salud|purif/i.test(p)) return ['99.9% pureza', 'Certificado OMS', 'Entrega express'];
  if (/delivery|restaurante|comida/i.test(p)) return ['−40% tiempo', '+60% pedidos', 'Sin errores'];
  if (/ventas|crm|lead/i.test(p)) return ['3x más ventas', '−70% tiempo', '100% auto'];
  if (/analí|dato/i.test(p)) return ['Datos en vivo', 'IA predictiva', 'Sin código'];
  return ['Resultado día 1', 'Setup 5 min', 'ROI medible'];
}

function buildScenes(brand, problem, features, benefit, tone, sceneCount, rrss, promptHash) {
  const scenes = [];
  const hooks        = TONE_HOOKS[tone]        || TONE_HOOKS.Inspirador;
  const brandLines   = TONE_BRAND_LINES[tone]  || TONE_BRAND_LINES.Inspirador;
  const featLines    = TONE_FEATURE_LINES[tone] || TONE_FEATURE_LINES.Inspirador;
  const ctaLines     = TONE_CTA_LINES[tone]    || TONE_CTA_LINES.Inspirador;
  const stats        = buildBrandStats(problem + ' ' + benefit, benefit);
  const h            = promptHash;
  ...
  scenes.push({
    type: 'hook',
    title: tone === 'Urgente' ? '¡Atención!' : tone === 'Dramático' ? 'La realidad.' : '¿Te pasa esto?',
    subtitle: `Hay una solución para ${rrss}.`,
    badge: `📍 Para ${rrss}`,
    painPoints: buildPainPoints(problem, tone),
    voiceover: pick(hooks, h)(brand, problem),
  });

  // Scene 2: Brand reveal (if sceneCount >= 4)
  if (sceneCount >= 4) {
    const titleVariants = [
      `${brand} | ${tone === 'Urgente' ? 'lo resuelve' : 'lo cambia todo'}`,
      `Conoce ${brand}`,
      `${brand} — la solución`,
    ];
    scenes.push({
      type: 'brand',
      title: pick(titleVariants, h + 1),
      subtitle: benefit,
      badge: tone === 'Profesional' ? '🏆 Enterprise' : tone === 'Urgente' ? '⚡ Ya disponible' : '✨ Solución #1',
      items: stats,
      voiceover: pick(brandLines, h + 2)(brand, benefit),
    });
  }
  ...
  const featCount = Math.max(0, sceneCount - (sceneCount >= 4 ? 2 : 1) - 1);
  const featItems = [
    ['Sin configuración', 'Resultado inmediato', 'Incluido gratis'],
    ['Fácil de usar', '100% automatizado', 'Soporte incluido'],
    ['Plug & play', 'Sin código', 'Escala con vos'],
    ['Setup en 5 min', 'Sin tecnicismos', 'Probado y funciona'],
  ];
  for (let i = 0; i < Math.min(featCount, features.length); i++) {
    const feat = features[i];
    const badgeOptions = ['⚡ Automatización', '📊 Analytics', '🚀 Performance', '🔒 Seguridad', '🌐 Integración'];
    scenes.push({
      type: 'feature',
      title: feat,
      subtitle: `${brand} en acción`,
      badge: badgeOptions[(i + h) % badgeOptions.length],
      items: featItems[(i + h) % featItems.length],
      voiceover: pick(featLines, h + 3 + i)(feat, brand),
    });
  }
  ...
  const ctaSubtitles = [
    `${benefit}. Sin compromisos.`,
    `Empieza gratis. Cancela cuando quieras.`,
    `Tu negocio, transformado en minutos.`,
  ];
  scenes.push({
    type: 'cta',
    title: tone === 'Urgente' ? '¡Empieza AHORA!' : tone === 'Dramático' ? 'Es tu momento.' : 'Empieza | Gratis',
    subtitle: pick(ctaSubtitles, h + 5),
    badge: '🎯 Comenzar Gratis',
    items: [domain, '✓ Sin tarjeta de crédito'],
    voiceover: pick(ctaLines, h + 6)(brand),
  });

  return scenes;
}

function generateScript({ prompt, brandName = 'la solución', tone = 'Inspirador', durationSeconds = 30, rrss = 'TikTok', creativity = 50, brandDesc = '' }) {
  // Build full context: brand description + user prompt
  const context = [brandDesc, prompt].filter(s => s && s.trim().length > 5).join('. ');

  const category = detectCategory(context);
  const problem  = extractProblem(context);
  const benefit  = extractBenefit(context);
  const features = extractFeaturesContextual(context, category);
  const hash     = strHash(prompt + brandName + tone + String(creativity));

  const dur        = Number(durationSeconds);
  const sceneCount = dur <= 15 ? 3 : dur <= 21 ? 4 : dur <= 30 ? 4 : dur <= 45 ? 5 : 6;
  const boldness   = creativity / 100;

  const scenes = buildScenesPAS({ brandName, problem, features, benefit, category, tone, sceneCount, rrss, hash, boldness });

  // Assemble script (~2.4 words/sec for narration)
  const targetWords = Math.floor(dur * 2.4);
  const lines = scenes.map(s => s.voiceover).filter(Boolean);
  let script = lines.join(' ');
  const words = script.split(/\s+/);
  if (words.length > targetWords) script = words.slice(0, targetWords).join(' ') + '.';

  return { script, scenes, wordCount: script.split(/\s+/).length, problem, benefit, category };
}

// Industry-aware feature extractor
function extractFeaturesContextual(prompt, category) {
  const p = (prompt || '').toLowerCase();
  const features = [];
  const CAT_FEATURES = {
    water:   ['Filtración 5 etapas', 'Certificado OMS', 'Sin plástico de un uso', 'Entrega express'],
    food:    ['Comandas digitales', 'Caja integrada', 'Gestión de mesas', 'Pedidos online'],
    health:  ['Seguimiento personalizado', 'Agenda online', 'Historial digital', 'Recordatorios IA'],
    general: [],
  };
  if (CAT_FEATURES[category]) return CAT_FEATURES[category].slice(0, 3);
  return extractFeatures(prompt).slice(0, 3);
}
...
app.post('/api/script/generate', async (req, res) => {
  ...
  try {
    ...
    if (keys.GEMINI_API_KEY) {
      ...
      // Fallback: PAS local engine
      result = generateScript({ prompt, brandName: brandName || 'la marca', tone, durationSeconds, rrss, creativity: Number(creativity), brandDesc });
    }
    res.json({ success: true, ...result });
  } catch (err) {
    console.error('Script generate error:', err.message);
    // Fallback to local PAS engine on Gemini failure
    try {
      const result = generateScript({ prompt, brandName: brandName || 'la marca', tone, durationSeconds, rrss, creativity: Number(creativity), brandDesc });
      res.json({ success: true, ...result, engine: 'pas-fallback' });
    } catch (e2) {
  ...
  (async () => {
    try {
      ...
      let scenesForRender = (Array.isArray(scenesFromClient) && scenesFromClient.length)
        ? scenesFromClient
        : generateScript({ prompt: script, brandName: brand.displayName, durationSeconds: Number(durationSeconds), brandDesc: brand.description || '' }).scenes;
...
app.post('/api/click-to-ad', async (req, res) => {
  ...
  try {
    ...
    if (!scriptResult) {
      scriptResult = generateScript({ prompt: fullContext || `video para ${brandName}`, brandName, tone, durationSeconds, rrss, creativity: 75, brandDesc: fullContext });
    }
...
app.post('/api/ai-edit', async (req, res) => {
  ...
  if (keys.GEMINI_API_KEY) {
    try {
      ...
      const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash-8b', generationConfig: { temperature: 0.4, maxOutputTokens: 512, responseMimeType: 'application/json' } });
